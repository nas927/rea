...
<div class="h-full flex justify-center">
	<div class="w-1/2 lg:w-full lg:mx-4 grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-3 place-content-center gap-4">
		<label class="inside-grid flex flex-col more-than-2 justify-center items-center rounded-xl cursor-pointer shadow-xl">
			<input type="radio" ng-model="confirm" ng-change="update('@{{ tab_index[increment] }}', confirm)" value="@{{ tab[tab_index[increment]][0] }}"/>
			<span>Moins de 2 ans</span>
		</label>
		<label class="inside-grid flex flex-col more-than-2 justify-center items-center rounded-xl cursor-pointer shadow-xl">
			<input type="radio" ng-model="confirm" value="@{{ tab[tab_index[increment]][1] }}"  ng-change="update('@{{ tab_index[increment] }}', confirm)"/>
			<span>Entre 2 et 15 ans</span>
		</label>						
		<label class="inside-grid flex flex-col more-than-2 justify-center items-center rounded-xl cursor-pointer shadow-xl">
			<input type="radio" ng-model="confirm" value="@{{ tab[tab_index[increment]][2] }}"  ng-change="update('@{{ tab_index[increment] }}', confirm)"/>
